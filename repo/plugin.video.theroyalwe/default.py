...
CONTEXT_PRIORITIES = enum(WATCHFLAG=1, FAVORITE=30, WATCHLIST=30, FAVLIST=31, CUSTOMLIST=32, SIMILAR=11, SEASONS=12, TRAILER=10, CACHEFILE=41, PLAY_CACHE=42, DELETE_CACHE=43, SCRAPERS=90, FILTERS=91, VISIBILITY=92, SET_VIEW=95, WALTER=99, TRANSMOGRIFY=100, KAT=101, TM_STREAMING=105, TM_QUEUE=106, FIND_FANART=998, TOOLS=999, SETTINGS=1000)
...
def update_run():
	if plugin.check_version(plugin.get_setting('version'), '1.0.8'):
...
def tv_list():
	...
	for record in results:
		if plugin.mode != 'tv_favorites' and plugin.mode != 'tv_genre':
		...
		menu.add('Find or Update Fanart', {"mode": "update_series_fanart", "imdb_id": record['imdb_id'], "tvdb_id": record['tvdb_id'], "tmdb_id": record['tmdb_id'], "trakt_id": record['trakt_id'],  "fanart": record['backdrop_url']}, priority=CONTEXT_PRIORITIES.FIND_FANART, visible=validate_show_fanart)
...
def movie_list():
	...
	count = len(results)
	for record in results:
		if plugin.mode not in ['movie_favorites', 'movie_genre']:
		...
		menu.add('Find or Update Fanart', {"mode": "update_movie_fanart", "imdb_id": record['imdb_id'], "tmdb_id": record['tmdb_id'], "trakt_id": record['trakt_id'],  "fanart": record['backdrop_url']}, priority=CONTEXT_PRIORITIES.FIND_FANART, visible=validate_show_fanart)
...
def scraper_list():
	...
	for scraper in scrapers: 
		try:
			...
			premiumize = False if 'transmogrifier' in scraper['name'] else True
		
		name = "%s [COLOR pink](premiumize)[/COLOR]" % scraper['name'] if premiumize else scraper['name']
		name = "[COLOR red]%s[/COLOR]" % scraper['name'] if broken else name
		items.append(name)
	...
	class ScraperWindow(Window):
		...
		def disable_all(self):
			for index in xrange(0, self.get_object('scrapers').size()):
				...
				self.get_object('scrapers').getListItem(index).setProperty("state", "disabled")
				if name.startswith('['): 
					name = name.replace('[COLOR red]', '')
					name = name.replace('[/COLOR]', '')
		...
		def enable_all(self):
			for index in xrange(0, self.get_object('scrapers').size()):
				...
				name = self.get_object('scrapers').getListItem(index).getLabel()
				self.get_object('scrapers').getListItem(index).setProperty("state", "enabled")
				if name.startswith('['): 
					name = name.replace('[COLOR red]', '')
					name = name.replace('[/COLOR]', '')
...
def toggle_scraper():
	...
	name = plugin.args['scraper'].replace(" [COLOR pink](premiumize)[/COLOR]", "")
	Scraper.toggle_scraper_state(name)
...
def clear_cache(ok=False):
	...
	if ok:
		from dudehere.routines.trakt import TraktAPI
		trakt = TraktAPI()
		tables = ['activities', 'activity_cache', 'cache', 'metadata_activities', 'metadata_shows', 'metadata_episodes', 'metadata_movies']
		PB = ProgressBar()
...
def get_tv_favorites():
	...
	records = []
	DB.connect()
	results = DB.query("SELECT id, cache FROM my_favorites WHERE media='tvshow' ORDER BY title ASC", force_double_array=True)
	DB.disconnect()
	if results:
		for row in results:
			record = plugin.unpickle(str(row[1]))
			record['id'] = row[0]	
			records += [record]
		return records
	else:
...
def get_movie_favorites():
	...
	records = []
	DB.connect()
	results = DB.query("SELECT id, cache FROM my_favorites WHERE media='movie' ORDER BY title ASC", force_double_array=True)
	DB.disconnect()
	if results:
		for row in results:
			record = plugin.unpickle(str(row[1]))
...
def add_to_favorites():
	...
	if plugin.args['media'] == 'tvshow':
		SQL = "REPLACE INTO my_favorites (media, imdb_id, tmdb_id, tvdb_id, trakt_id, slug, title, cache) VALUES('tvshow',?,?,?,?,?,?,?)"
		DB.execute(SQL, [record['imdb_id'], record['tmdb_id'], record['tvdb_id'], record['trakt_id'], record['slug'], record['title'], plugin.pickle(record)])
	else:
		SQL += "REPLACE INTO my_favorites (media, imdb_id, tmdb_id, trakt_id, slug, title, cache) VALUES('movie',?,?,?,?,?,?)"
		DB.execute(SQL, [record['imdb_id'], record['tmdb_id'], record['trakt_id'], record['slug'], record['title'], plugin.pickle(record)])
	DB.commit()
	DB.disconnect()	
...
def delete_from_favorites():
	...
	if ok:
		DB.connect()
		DB.execute("DELETE FROM my_favorites WHERE id=? AND media=?", [plugin.args['id'], plugin.args['media']])
		DB.commit()
		DB.disconnect()
...
if __name__ == '__main__':
	plugin.log(plugin.get_property('theroyalwe.last_directory'))
	plugin.run()
